...
class Net3(Module):
    def __init__(self,
        ...
        self.embedding = Embedding(256, self.embedding_dim)
        ...
        self.decoder = Linear(self.num_hidden2, 256)
    ...
    def forward(self, batch):
        ...
        x = torch.sigmoid(x) # x.shape == [..., self.num_hidden2]
    ...
    def probs(self, X):
        ...
        x = x.view(-1,self.context_length*self.embedding_dim) # x.shape == [..., self.context_length*self.embedding_dim]
        x = self.encoder(x) # x.shape == [..., self.num_hidden1]
        x = torch.sigmoid(x) # x.shape == [..., self.num_hidden1]
        x = self.middle(x) # x.shape == [..., self.num_hidden2]
        x = torch.sigmoid(x) # x.shape == [..., self.num_hidden2]
        x = self.decoder(x) # x.shape == [..., 256]
